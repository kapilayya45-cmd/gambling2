...
exports.deployCoins = functions.https.onCall(async (data, context) => {
    ...
    if (!context || !context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'You must be signed in to deploy coins.');
    }
    const callerUid = context.auth.uid;
    // Ensure the callerUid matches fromUid
    if (fromUid !== callerUid) {
        throw new functions.https.HttpsError('permission-denied', 'You can only deploy coins from your own account.');
    }
    // 2) Validate amount
    ...
    if (callerRole !== 'superadmin' && callerRole !== 'admin') {
        throw new functions.https.HttpsError('permission-denied', 'Insufficient privileges to deploy coins.');
    }
    // 5) Balance check (skip for superadmins)
    if (!isSuperadmin) {
        const callerBalance = callerData.coinBalance || 0;
        if (callerBalance < amount) {
            throw new functions.https.HttpsError('failed-precondition', `Insufficient coins (have ${callerBalance}, need ${amount}).`);
        }
    }
    ...
    const userQuery = await admin
        ...
        .get();
    if (userQuery.empty) {
        throw new functions.https.HttpsError('not-found', `No user found with email "${toEmail}".`);
    }
    ...
    if (callerRole === 'superadmin' && targetRole !== 'admin') {
        throw new functions.https.HttpsError('permission-denied', 'Superadmin may only top up admins.');
    }
    if (callerRole === 'admin' && targetRole !== 'user') {
        throw new functions.https.HttpsError('permission-denied', 'Admin may only top up regular users.');
    }
