...
export const IPLMatchCard: React.FC<IPLMatchCardProps> = ({ match, showOdds = true }) => {
  ...
  const homeTeamOdds = match.betting_odds?.match_winner?.[match.localteam_name];
  const awayTeamOdds = match.betting_odds?.match_winner?.[match.visitorteam_name];
  ...
  // Create odds and volumes objects for MatchWinnerUI
  const matchWinnerOdds: Record<string, number> = {};
  const matchWinnerVolumes: Record<string, number> = {};
  
  if (homeTeamOdds) {
    matchWinnerOdds[match.localteam_name] = homeTeamOdds;
    // Mock volume data, replace with actual data if available
    matchWinnerVolumes[match.localteam_name] = Math.floor(Math.random() * 5000) + 1000;
  }
  
  if (awayTeamOdds) {
    matchWinnerOdds[match.visitorteam_name] = awayTeamOdds;
    // Mock volume data, replace with actual data if available
    matchWinnerVolumes[match.visitorteam_name] = Math.floor(Math.random() * 5000) + 1000;
  }
    ...
    <div className={`border rounded-lg p-4 mb-4 shadow-sm ${isLive ? 'border-red-500 bg-red-50' : isCompleted ? 'border-gray-300 bg-gray-50' : 'border-blue-300 bg-blue-50'}`}>
        ...
        <div className="flex flex-col items-start w-5/12">
          <div className="font-bold text-lg flex items-center">
            <span className="mr-2 bg-gray-200 text-gray-800 rounded px-1 text-sm">
              {match.short_title ? match.short_title.split(' vs ')[0] : 'HOME'}
            </span>
        ...
        <div className="flex flex-col items-end w-5/12">
          <div className="font-bold text-lg flex items-center justify-end">
            ...
            <span className="ml-2 bg-gray-200 text-gray-800 rounded px-1 text-sm">
              {match.short_title ? match.short_title.split(' vs ')[1] : 'AWAY'}
            </span>
          </div>
      ...
      {showOdds && (homeTeamOdds || awayTeamOdds) && (
        <div className="mt-3 pt-3 border-t border-gray-200">
          ...
          <MatchWinnerUI odds={matchWinnerOdds} volumes={matchWinnerVolumes} />
        </div>
      )}
